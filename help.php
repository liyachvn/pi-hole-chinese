...
<div class="row">
    ...
    <ul>
        ...
        <li>温度：当前 CPU 温度</li>
        <li>负载：分别为最后 1 分钟、5 分钟和 15 分钟的平均负载。平均负载为 1 反映了系统上单个处理器的全部工作负载。如果当前负载超过此计算机上的可用处理器数量。(为 <?php echo $nproc; ?>)，我们将显示一个红色图标。</li>
        <li>内存使用：显示实际被应用程序阻止的内存百分比。如果内存使用率超过 75%，我们将显示一个红色图标。</li>
    </ul>
    <h4>顶部右侧：关于</h4>
    <ul>
        <li>GitHub: 链接到 Pi-hole 仓库。</li>
...
<div class="row">
    <div class="col-md-12">
    <h2>仪表板</h2>
    <p>在仪表板上，您可以查看各种 Pi-hole 统计信息：</p>
    <ul>
        <li>摘要：统计信息摘要显示了今天被阻止的 DNS 查询总数，被阻止的 DNS 查询百分比以及已编译的广告列表中有多少个域。此摘要每 10 秒更新一次。</li>
        <li>一段时间内的查询：该图显示了 10 分钟时间间隔内的 DNS 查询（总计和已阻止）。悬停在和行上可以获取更多信息。该图每 10 分钟更新一次。</li>
        <li>查询类型：标识已处理查询的类型。</li>
        <li>转发目标：显示允许的请求已转发到哪个上游 DNS。</li>
        <li>活跃域名：按 DNS 查找数对请求的网站进行排名。</li>
        <li>活跃广告：按 DNS 查找数对请求的广告进行排名。</li>
        <li>活跃客户端：每个客户端在本地网络上发出的 DNS 请求数据的排名。</li>
    </ul>
    <p>根据设置页面上的隐私设置，“活跃域名”和“活跃广告”列表可能会隐藏。</p>
    <?php if($authenticationsystem){ ?>
...
<div class="row">
    <div class="col-md-12">
    <h2>查询日志</h2>
    <p>通过分析 Pi-hole 的日志显示最近的查询。通过在“搜索”输入框键入关键字可以搜索整个列表。如果状态报告为“确定”，则表明 DNS 请求已被允许。否则 ("Pi-holed") 它已被阻止。通过单击“动作”下的按钮，可以将相应的域快速添加到白名单/黑名单中。操作状态将在此页面上报告。默认情况下，仅显示最近的 10 分钟以提高查询日志页面的加载速度。通过单击页面标题中的相应链接，可以请求所有域。请注意，结果在很大程度上取决于您的隐私设置（请参阅“设置”页面）。</p>
    </div>
</div>
<div class="row">
    <div class="col-md-12">
...
<div class="row">
    <div class="col-md-12">
    <h2>捐助</h2>
    请记住，Pi-hole 是免费的。如果您喜欢 Pi-hole，请考虑捐款以支持其发展。
    </div>
</div>
<div class="row">
    <div class="col-md-12">
