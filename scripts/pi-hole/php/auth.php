...
function check_cors() {
    ...
    if(isset($_SERVER['HTTP_ORIGIN'])) {
        ...
        if(!in_array($server_origin, $AUTHORIZED_HOSTNAMES)) {
            log_and_die("CORS 失败：" . $server_origin .' vs '. join(', ', $AUTHORIZED_HOSTNAMES));
        }
...
function check_domain(&$domains) {
    foreach($domains as &$domain)
    {
        ...
        if(!$validDomain){
            log_and_die(htmlspecialchars($domain. ' 是一个无效的域。'));
        }
    }
}
...
function list_verify($type) {
    ...
    elseif(isset($_POST['pw']))
    {
        ...
        {
            log_and_die("密码错误 - ".htmlspecialchars($type)."listing of ".htmlspecialchars($_POST['domain'])." 不允许。");
        }
    }
    else
    {
