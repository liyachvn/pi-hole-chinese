...
function add_to_table($db, $table, $domains, $comment=null, $wildcardstyle=false, $returnnum=false, $type=-1)
{
	if(!is_int($type))
	{
		return "错误：参数类型必须是整数类型（是 ".gettype($type)."）";
	}
	...
	if(!$stmt)
	{
		if($returnnum)
			...
			return "错误：无法为 $table 表准备语句（类型 = $type, 字段 = $field).";
	}
		...
		else
		{
			...
			else
			{
				if($num === 1)
					...
					$plural = "s";
				return "错误：".$db->lastErrorMsg().", 已添加到 ".$num." 域".$plural;
			}
		}
	}
	...
	else
	{
		...
		if($num === 1)
			...
			$plural = "s";
		return "成功，已经添加 ".$modified." 到 ".$num." 域".$plural.$extra;
	}
}
...
function remove_from_table($db, $table, $domains, $returnnum=false, $type=-1)
{
	if(!is_int($type))
	{
		return "错误：参数类型必须为整数类型（是 ".gettype($type)."）";
	}
	...
	if(!$stmt)
	{
		if($returnnum)
			...
			return "错误：无法为 ".$table." 表准备语句（类型 = ".$type.").";
	}
		...
		else
		{
			...
			else
			{
				if($num === 1)
					...
					$plural = "s";
				return "错误：".$db->lastErrorMsg().", 已移除 ".$num." 域".$plural;
			}
		}
	}
